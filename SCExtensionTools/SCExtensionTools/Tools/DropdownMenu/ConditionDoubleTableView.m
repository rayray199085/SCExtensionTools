...
- (void)showTableView:(NSInteger)index WithSelectedLeft:(NSString *)left Right:(NSString *)right {
    if (isHidden == YES || _buttonIndex != index) {
        ...
        [UIView animateWithDuration:0.5 animations:^{
            self->_rootView.center = CGPointMake(self.view.frame.size.width / 2, self->_rootView.bounds.size.height / 2);
        }];
    } else {
        [self hideTableView];
    }
...
- (void)hideTableView {
    ...
    [UIView animateWithDuration:0.5 animations:^{
        self.view.alpha = 0.0;
         ...
         [self.view setHidden:YES];
        self->_rootView.center = CGPointMake(self.view.frame.size.width / 2, 0 - self->_rootView.bounds.size.height / 2);
        [[NSNotificationCenter defaultCenter] postNotificationName:@"hideMenu" object:[NSString stringWithFormat:@"%ld",(long)self->_buttonIndex]];
    }];
}
