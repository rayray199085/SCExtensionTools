//
//  VCLLogingViewController.swift
//  Concentration
//
//  Created by Alevtina on 17/04/2019.
//  Copyright © 2019 Alevtina. All rights reserved.
//

import UIKit

class VCLLogingViewController: UIViewController {
    
    private struct LogGlobals {
        var prefix = ""
        var instanceCount = [String : Int]()
    ...
    private static func bumpInstanceCount(for loggingName: String) -> Int {
        logGlobals.instanceCount[loggingName] = (logGlobals.instanceCount[loggingName] ?? 0) + 1
        return logGlobals.instanceCount[loggingName]!
    }
    ...
    private func logVCL(_ msg: String) {
        if instanceCount == nil {
            instanceCount = VCLLogingViewController.bumpInstanceCount(for: vclLoggingName)
        }
        print("\(VCLLogingViewController.logPrefix(for: vclLoggingName))(\(instanceCount!)) \(msg)")
    }
